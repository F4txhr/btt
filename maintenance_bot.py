...
async def main():
    ...
    pid_manager = PIDManager("maintenance_bot.pid")
    with pid_manager as pid_is_new:
        if not pid_is_new:
            logger.error("Maintenance bot sudah berjalan dengan PID lain. Keluar.")
            return
        ...
        try:
            ...
            for f in ["maintenance_info.json", "maintenance_job.json"]:
                if os.path.exists(f):
                    try:
                        os.remove(f)
                    except OSError as e:
                        logger.error(f"Gagal menghapus file temporary '{f}': {e}")
